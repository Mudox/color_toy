...
function s:core.getCurColor() dict                                                "    {{{2
  if ! exists('g:colors_name')
    throw 'g:colors_name is not set'
  endif
...
function s:core.showCurColors() dict                                              "    {{{2
  ...
  let msg            = printf("current color: %s #%d in [%s]",
        \ name, points, context_string)

  let color_from = self.lastLastColor
  let color_to = self.lastColor
  echo printf("color switched: %s[%d] -> %s[%d]",
        \ color_from, self.getPoint(self.getCurContext(), color_from),
        \ color_to, self.getPoint(self.getCurContext(), color_to)
        \ )
